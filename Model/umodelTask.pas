...
begin
  ...
  {Criação da query e execução do SELECT}
  lQry := TConnection.Query;
  lQry.Add('SELECT * FROM TASK');
  lQry.Open;

  {Percorrendo os registros para montar o array do json}
...
var lQry: iQuery;
begin
{{Criação da query e SELECT}
  lQry := TConnection.Query;
  lQry.Add('SELECT * FROM TASK WHERE CODIGO = :CODIGO');
  lQry.AddParam('CODIGO', aCodigo);
  lQry.Open;

  {Verificação do resultado e criação do objeto de resposta}
...
var lQry: iQuery;
begin
  {Criação da query e UPDATE do status da tarefa}
  lQry := TConnection.Query;
