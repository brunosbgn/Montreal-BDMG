...
begin
  try
    {Requisição no serviço - para as demais, considerar os mesmos comentários}
    ...
    begin
      {Parse do objeto para inserir no campo}
      lObj := TJSONObject.ParseJSONValue(LResponse.Content) as TJSONObject;
      try
        edtResultado.Text := 'Tarefas concluídas (últimos 7 dias): ' +  lObj.GetValue('concluidas_7_dias').Value;
      finally
...
begin
  try
    ...
    begin
      lObj := TJSONObject.ParseJSONValue(LResponse.Content) as TJSONObject;
      try
        edtResultado.Text := 'Média de Prioridade (Pendentes): ' +  lObj.GetValue('media_prioridade').Value;
      finally
        FreeAndNil(lObj);
      end;
    end
...
begin
  try
    ...
    begin
      lObj := TJSONObject.ParseJSONValue(LResponse.Content) as TJSONObject;
      try
        edtResultado.Text := 'Total de tarefas: ' +  lObj.GetValue('total_tarefas').Value;
      finally
        FreeAndNil(lObj);
      end;
    end
