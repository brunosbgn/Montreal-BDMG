...
begin
  ...
  begin
    ShowMessage('Insira a descrição da tarefa!');
    Exit;
  end;
  ...
  try
    ...
    try
      ...
      {Verificação da reposta para mensagem}
      if LResponse.StatusCode = 200 then
        ShowMessage('Registro enviado!')
      else
        raise Exception.Create('Falha na inclusão do registro!');
    finally
      {Forma de liberação da memória}
      lObj := nil;
      lObj.Free;
      Self.Close;
    end;
