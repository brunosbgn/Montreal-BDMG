...
begin
  ...
  {Respondendo a requisição com o array}
...
var
  ...
  lObj: TJSONObject;
begin
  {Chamada do SELECT do desafio - considerar os comentários para as 3}
...
var
  ...
  lCodigo: Integer;
begin
  {Verificação do parâmetro, por ser consulta única}
  if aReq.Params.Count = 0 then
    raise Exception.Create('Código não encontrado!');

  {Alimentando a variável com o parâmetro e chamando o SELECT}
  lCodigo := aReq.Params.Items['codigo'].ToInteger;
  lTask := TmodelTask.SelectWhere(lCodigo);

  {Verificando se existe resultado para montar a resposta positiva e negativa}
  if Assigned(lTask) then
  begin
    lObj := TJSONObject.Create;
    lObj.AddPair('codigo', TJSONNumber.Create(lTask.codigo));
...
var
  ...
  lObj: TJSONObject;
begin
  {Verificação do corpo. Nesse teste, não criei muita tratativa aqui no serviço, pois obriguei no client}
  if aReq.Body.IsEmpty then
    raise Exception.Create('Requisição sem corpo!');

  {Leitura do corpo para dar o INSERT na tabela}
...
var
  ...
  lCodigo: Integer;
begin
  {Verificação do parâmetro e corpo da requisição}
  if aReq.Params.Count = 0 then
    raise Exception.Create('Código não encontrado!');
  ...
  {Criação do objeto simples e chamada do SELECT para capturar o código}
  lObj := aReq.Body<TJSONObject>;    
  lTask := TmodelTask.SelectWhere(lCodigo);

  {Resposta positiva e negativa, dependendo do UPDATE executado}
  if Assigned(lTask) then
  begin     
    lTask.status := lObj.GetValue<String>('status');
    lTask.Update;  
